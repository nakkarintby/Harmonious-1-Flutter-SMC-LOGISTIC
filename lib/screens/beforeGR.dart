...
class BeforeGRPage extends StatefulWidget {
  @override
  _BeforeGRPageState createState() => _BeforeGRPageState();
}

class _BeforeGRPageState extends State<BeforeGRPage> {
  ...
  Future<void> documentIDCheck() async {
    ...
    if (response.statusCode != 200) {
      showErrorDialog('Error Http Requests documentIDCheck Before');
      return;
    }
  ...
  Future<void> upload() async {
    ...
    print("call post api upload image before");
    ...
    if (response.statusCode != 200) {
      ...
      showErrorDialog('Error Http Requests upload1 Before');
      return;
    }

    print("success call post api upload image before");
    ...
    var url2 = Uri.parse(configs +
        ...
        '/' +
        eventType);
    print("call get api check upload image before");
    http.Response response2 = await http.get(url2);

    if (response.statusCode != 200) {
      await showProgressLoading(true);
      showErrorDialog('Error Http Requests upload2 Before');
      return;
    }

    print("success call get api check upload image before");
    ...
    if (sequence == 1) {
      ...
      print("call post api update document when sequence = 1 before");
      ...
      if (response.statusCode != 200) {
        await showProgressLoading(true);
        showErrorDialog('Error Http Requests upload3 Before');
        return;
      }

      print("success call post api update document when sequence = 1 before");
  ...
  Future<void> finish() async {
    ...
    if (response.statusCode != 200) {
      await showProgressLoading(true);
      showErrorDialog('Error Http Requests finish Before');
      return;
    }
